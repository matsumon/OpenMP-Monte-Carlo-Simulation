...
main( int argc, char *argv[ ] )
{
	...
	// const int NUMT=atoi(argv[1]);
	// const int NUMTRIALS = atoi(argv[2]);
	...
	double maxTime = 0.;	// must be declared outside the NUMTRIES loop
	...
	for( int tries = 0; tries < NUMTRIES; tries++ )
	{
		...
		#pragma omp parallel for default(shared)  reduction(+:numHits)
		for( int n = 0; n < NUMTRIALS; n++ )
		{
	...
	fprintf(stderr, "%2d,%8d,%6.2f%%,%6.2lf,%1.7lf\n",
		NUMT, NUMTRIALS, 100.*probability, maxPerformance, maxTime);
